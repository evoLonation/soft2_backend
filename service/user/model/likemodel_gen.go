...
var (
	...
	likeRowsExpectAutoSet   = strings.Join(stringx.Remove(likeFieldNames, "`like_id`", "`created_at`", "`create_time`", "`update_at`", "`updated_at`", "`update_time`", "`create_at`"), ",")
	likeRowsWithPlaceHolder = strings.Join(stringx.Remove(likeFieldNames, "`like_id`", "`created_at`", "`create_time`", "`update_at`", "`updated_at`", "`update_time`", "`create_at`"), "=?,") + "=?"
...
func (m *defaultCollectModel) FindLikeId(ctx context.Context, userId int64, commentId int64) (*Collect, error) {
	collectCollectIdKey := fmt.Sprintf("%s%v%v", cacheCollectCollectIdPrefix, userId, commentId)
	var resp Collect
	err := m.QueryRowCtx(ctx, &resp, collectCollectIdKey, func(ctx context.Context, conn sqlx.SqlConn, v interface{}) error {
		query := fmt.Sprintf("select %s from %s where `user_id` = ? and `comment_id` =? limit 2", collectRows, m.table)
