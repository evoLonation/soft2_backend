...
func (m *defaultCollectModel) FindSubscribeId(ctx context.Context, userId int64, scholarId int64) (*Collect, error) {
	collectCollectIdKey := fmt.Sprintf("%s%v%v", cacheCollectCollectIdPrefix, userId, scholarId)
	var resp Collect
	err := m.QueryRowCtx(ctx, &resp, collectCollectIdKey, func(ctx context.Context, conn sqlx.SqlConn, v interface{}) error {
		query := fmt.Sprintf("select %s from %s where `user_id` = ? and `scholar_id` =? limit 2", collectRows, m.table)
